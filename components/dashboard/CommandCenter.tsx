...
export default function CommandCenter({
  ...
  const hasWorkflows = savedWorkflows > 0;
      ...
      <div className="relative grid gap-6 xl:grid-cols-[1.4fr_.8fr] xl:items-center">
        <div>
          <div className="flex items-center gap-2 text-xs font-medium text-violet-200">
            <Sparkles className="size-4" /> Workflow command center
          </div>
          <h2 className="mt-3 max-w-2xl text-2xl font-semibold tracking-tight sm:text-3xl">
            {hasWorkflows ? "Your automation workspace is taking shape." : "Create your first real Nexora workflow."}
          </h2>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-white/45">
            {hasWorkflows
              ? "These numbers now come from your Supabase workspace. Build, pause, and manage workflows without leaving Nexora."
              : "Start with a trigger and an action. Nexora will store it securely under your signed-in account."}
          </p>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            ...
            >
              New workflow <ArrowRight className="size-4" />
            </Link>
            <Link
        ...
        <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
          {[
            { icon: Workflow, label: "Active workflows", value: String(activeWorkflows), tone: "text-violet-200 bg-violet-400/10" },
            { icon: CheckCircle2, label: "Saved workflows", value: String(savedWorkflows), tone: "text-emerald-200 bg-emerald-400/10" },
            { icon: Zap, label: "Runs tracked", value: trackedRuns.toLocaleString(), tone: "text-amber-200 bg-amber-400/10" },
